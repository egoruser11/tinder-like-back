...
## Лента: каркас для разработки

Все маршруты ленты находятся под JWT middleware: middleware кладёт `user_id`
в Gin context, а handler передаёт его в сервис. Алгоритм выборки, фильтрации,
ранжирования и записи действий намеренно не реализован: он должен быть написан
в [internal/service/ribbon.go](go-core/internal/service/ribbon.go).

- `GET /api/v1/ribbon`
- `GET /api/v1/ribbon/likes`
- `POST /api/v1/ribbon/likes`
- `POST /api/v1/ribbon/dislikes`
- `POST` / `DELETE /api/v1/ribbon/blocks`
- `POST /api/v1/ribbon/reports`

Пока методы возвращают `501 Not Implemented`: это подтверждает, что API-каркас
подключён, но не подменяет бизнес-логику.
